...
function BibleImportModal({ 
  ...
  const [quickReference, setQuickReference] = useState<string>(""); // rýchle zadanie odkazu
  const [useQuickMode, setUseQuickMode] = useState<boolean>(true); // prepínač režimov - predvolene TRUE
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setPreviewText("");
      setQuickReference("");
      setUseQuickMode(true); // predvolene rýchly režim
      setBooks([]);
      setChapters([]);
      setVerses([]);
    }
  ...
  // Parse quick reference like "Lk 23, 35-43" or "Mt 4, 12-17. 23-25"
  const parseQuickReference = (ref: string): { 
    ...
    // Match patterns: "Lk 23, 35-43" or "Mt 4, 12-17. 23-25" or "1Kor 13, 1-13"
  ...
  // Handle quick reference processing
  const handleQuickReference = async () => {
    ...
    try {
      ...
      if (!error && data) {
        const loadedVerses = data.map((v: any) => ({
          id: v.id,
          chapter: v.chapter,
          verse: v.verse,
          text: v.text,
  ...
  const handleImport = () => {
    ...
    // V rýchlom režime možno nemáme selectedBook/selectedChapter nastavené,
    // ale máme verses s informáciami
    let book, chapter;
    ...
    if (selectedBook && selectedChapter) {
      ...
      // Použijeme info z načítaných veršov
      book = verses[0].book;
    ...
    console.log('Importing:', { previewText, reference, translation: translation?.name });
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden">
        ...
        <div className="p-6 overflow-y-auto max-h-[calc(90vh-80px)]">
          ...
          {/* Rýchly režim - priame zadanie odkazu */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
            ...
            {/* Multi-range input - iba pre ručný režim */}
            <div className="mb-6">
          ...
          {/* Náhľad textu - zobrazí sa v oboch režimoch */}
          {previewText && (
            <div className="mb-6">
              <label className="block text-sm font-semibold text-gray-700 mb-2">
                Náhľad textu
